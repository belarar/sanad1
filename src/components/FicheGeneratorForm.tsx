      ...
      <div className="border-b border-slate-100 pb-5 mb-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            <span className="text-xs text-slate-500 font-medium">نماذج تطبيقية كاملة:</span>
            {PREBUILT_FICHES.slice(0, 6).map((fiche) => {
              const label =
                fiche.header.subject.includes("علوم")
                  ? "علوم 4م"
                  : fiche.header.subject.includes("فيزياء")
                  ? "فيزياء 4م"
                  : fiche.header.subject.includes("رياضيات")
                  ? "رياضيات 4م"
                  : fiche.header.subject.includes("عربية")
                  ? "عربية 4م"
                  : fiche.header.subject.includes("تاريخ")
                  ? "تاريخ 4م"
                  : fiche.header.subject.includes("إسلامية")
                  ? "إسلامية 4م"
                  : fiche.header.subject.includes("مدنية")
                  ? "مدنية 4م"
                  : fiche.header.subject.includes("Français") || fiche.header.language === "fr"
                  ? "فرنسية 4م (FR)"
                  : fiche.header.subject.includes("English") || fiche.header.language === "en"
                  ? "إنجليزية 4م (EN)"
                  : fiche.header.subject.substring(0, 8);

              return (
                <button
                  key={fiche.id}
                  type="button"
                  onClick={() => onSelectPrebuilt(fiche)}
                  className="px-2.5 py-1 text-xs font-bold bg-blue-50 hover:bg-blue-600 hover:text-white text-blue-700 rounded-lg border border-blue-200 transition-colors cursor-pointer shadow-2xs"
                  title={`${fiche.header.subject}: ${fiche.header.resourceTitle}`}
                >
                  {label}
                </button>
              );
            ...
            {onOpenGallery && (
              <button
                ...
                className="px-2.5 py-1 text-xs font-bold bg-amber-50 hover:bg-amber-100 text-amber-800 rounded-lg border border-amber-300 transition-colors cursor-pointer flex items-center gap-1"
                title="استعراض بنك المذكرات الكامل بجميع المواد"
              >
                <BookOpen className="w-3 h-3 text-amber-600" />
                <span>عرض جميع النماذج ({PREBUILT_FICHES.length})</span>
              </button>
            )}
          </div>
        </div>
